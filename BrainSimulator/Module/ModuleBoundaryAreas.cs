    ...
    public class ModuleBoundaryAreas : ModuleBase
    {
            ...
            for (int i = 0; i < boundaries.Count; i++)
            {
                ...
                for (int j = i + 1; j < boundaries.Count; j++)
                {
                        ...
                        )
                    {
                        ...
                        {
                            跨语言接口.FindIntersection(boundaries[i].p1, boundaries[i].p2, boundaries[j].p1, boundaries[j].p2, out Point intersection);
        ...
        private void GetAreaCentroids()
        {
            foreach (Area a in areas)
            {
                ...
                a.centroid = 跨语言接口.GetCentroid(pts);
            }
        }
        ...
        private HSLColor GetAreaColor(Area a)
        {
            ...
            模块视图 source = theNeuronArray.FindModuleByLabel("ImageZoom");
            if (source == null) return retVal;

            if (a.areaCorners.Count == 2)
            {
                if (source.GetNeuronAt((int)a.areaCorners[0].loc.X, (int)a.areaCorners[0].loc.Y) is 神经元 n0)
                {
                    System.Drawing.Color c2 = 跨语言接口.IntToDrawingColor(n0.LastChargeInt);
                    retVal = new HSLColor(c2.GetHue(), c2.GetSaturation(), c2.GetBrightness());
                }
            }
            else
            {
                if(source.GetNeuronAt((int)a.centroid.X, (int)a.centroid.Y) is 神经元 n1)
                {
                    System.Drawing.Color c2 = 跨语言接口.IntToDrawingColor(n1.LastChargeInt);
                    retVal = new HSLColor(c2.GetHue(), c2.GetSaturation(), c2.GetBrightness());
                }
            }
            return retVal;
...
#pragma warning disable 162
            ...
            for (int i = minx; i < maxx; i++)
                ...
                {
                    if (跨语言接口.IsPointInPolygon(pts.ToArray(), new Point(i, j)))
                    {
                        if (source.GetNeuronAt(i, j) is 神经元 n)
                        {
                            System.Drawing.Color c1 = 跨语言接口.IntToDrawingColor(n.LastChargeInt);
                            pointCount++;
                            hueTot += c1.GetHue();
