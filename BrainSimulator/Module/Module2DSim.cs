...
namespace BrainSimulator.Modules
{
    public class Module2DSim : ModuleBase
    {
        ...
        public override void Fire()
        {
            ...
            模块视图 naArmL = theNeuronArray.FindModuleByLabel("ModuleArmL");
            模块视图 naArmR = theNeuronArray.FindModuleByLabel("ModuleArmR");
        ...
        private bool CheckForCollisions(Point newPosition)
        {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                ...
                double dist = 跨语言接口.FindDistanceToSegment(newPosition, ph.P1, ph.P2, out Point closest);
                if (dist < bodyRadius)
                {
        ...
        {
            模块视图 naSmell = theNeuronArray.FindModuleByLabel("Module2DSmell");
        ...
        private void HandleTouch(Point armPositionAbs, int index)
        {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                ...
                Point P2 = objects[i].P2;
                physObject ph = objects[i];
                跨语言接口.FindIntersection(P1, P2, entityPosition, armPositionAbs, out bool linesintersect, out bool segments_intersect,
                    out Point Intersection, out Point close_p1, out Point close_p2, out double collisionAngle);
        ...
        private void HandleVision(int row)
        {
            ...
            for (int i = 0; i < retinaWidth; i++)
            {
                ...
                for (int j = 0; j < objects.Count; j++)
                {
                    跨语言接口.FindIntersection(entityPosition, p2, objects[j].P1, objects[j].P2,
                ...
                pixels[i] = 跨语言接口.ColorToInt(theColor);
...
,        };
        ...
        private void DoTrainingAction()
        {
            ...
            if (trainingAction.IndexOf("[color]") != -1)
            {
                ...
                if (cL != cR) return;
                string colorName = 跨语言接口.GetColorName(跨语言接口.IntToColor(cL)).ToLower();
        ...
        public double GetColorWeightAtPoint(Point point, Color theColor)
        {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                if (objects[i].theColor == theColor)
                {
                    Point P1 = objects[i].P1;
                    Point P2 = objects[i].P2;
                    double distance = 跨语言接口.FindDistanceToSegment(point, P1, P2, out Point closest);
        ...
        public void SetModel()
        {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                ...
                int theColor = 跨语言接口.ColorToInt(objects[i].theColor);
        ...
        public override void Initialize()
        {
            ...
            mv.GetNeuronAt(0).标签名 = "Actions";
            mv.GetNeuronAt(1).标签名 = "Colors";
            mv.GetNeuronAt(2).标签名 = "Train";
            ...
            foreach (神经元 n in mv.Neurons)
            {
                ...
                foreach (突触 s in n.Synapses)
                {
                    if (s.TargetNeuron != n.Id)// && s.Weight != 10)
                    {
                        ...
                        currentColorInt--;
                        currentColor = 跨语言接口.IntToColor(currentColorInt);
                    }
                }
            }
            HandleVision();
