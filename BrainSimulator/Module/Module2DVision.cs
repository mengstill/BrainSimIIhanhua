...
namespace BrainSimulator.Modules
{
    public class Module2DVision : ModuleBase
    {
        ...
        private PointPlus FindDepth(int l, int r)
        {
            ...
            跨语言接口.FindIntersection(p1L.P, p2L.P, p1R.P, p2R.P, out bool lines_intersect, out bool segments_intersect, out Point intersection, out Point clost_p1, out Point close_p2, out double collisionAngle);
        ...
        public override void Initialize()
        {
            ...
            for (int i = 0; i < mv.Width; i++)
            {
                mv.GetNeuronAt(i, 0).Model = 神经元.模型类型.Color;
                mv.GetNeuronAt(i, 1).Model = 神经元.模型类型.Color;
            }
            mv.GetNeuronAt(0, 0).标签名 = "Left";
            mv.GetNeuronAt(0, 1).标签名 = "Right";
            mv.GetNeuronAt(mv.Width / 2, 0).标签名 = "  ||";
            lastValuesL = null;
            lastValuesR = null;
