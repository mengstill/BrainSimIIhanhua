...
namespace BrainSimulator.Modules
{
    public class ModuleGraph : ModuleBase
    {
        ...
        public void AddNode(string parent, string name, string[] attribs = null, string[] sequenceItems = null)
        {
            ...
            if (name != "." )
                mv.GetNeuronAt(Array.IndexOf(cols, "say"), newRow).添加突触(GetSpokenWord(name).Id, 1, 突触.modelType.Fixed);
            ...
            {
                mv.GetNeuronAt("'1'").添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "match"), newRow).Id, -(attribs.Length - 1), 突触.modelType.Fixed);
                for (int j = 0; j < mv.Height; j++)
                {
                    if (mv.GetNeuronAt(0, j).标签名 == attribs[i])
                    {
                        mv.GetNeuronAt(Array.IndexOf(cols, "attrib"), newRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "alt"), j).Id, 1, 突触.modelType.Fixed);
                        mv.GetNeuronAt(Array.IndexOf(cols, "allAttr"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "match"), newRow).Id, 1, 突触.modelType.Fixed);
                        mv.GetNeuronAt(Array.IndexOf(cols, "anyAttr"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "thing"), newRow).Id, 1, 突触.modelType.Fixed);
                        mv.GetNeuronAt(Array.IndexOf(cols, "attrib"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "alt"), newRow).Id, 1, 突触.modelType.Fixed);
            ...
            for (int i = 0; sequenceItems != null && i < sequenceItems.Length && nextRow < mv.Height; i++)
            {
                ...
                for (int j = 0; j < mv.Height; j++)
                {
                    if (mv.GetNeuronAt(0, j).标签名 == sequenceItems[i])
                    {
                        ...
                        //the initial entry
                        mv.GetNeuronAt(Array.IndexOf(cols, "in"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "match"), curRow).Id, 1, 突触.modelType.Fixed);
                        mv.GetNeuronAt(Array.IndexOf(cols, "in"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "match"), curRow).Id, -1, 突触.modelType.Fixed);
                        mv.GetNeuronAt(Array.IndexOf(cols, "match"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), curRow).Id, 1, 突触.modelType.Fixed);

                        //subsequent entries
                        if (prevRow >= 0)
                            mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "nMtch"), prevRow).Id, 1, 突触.modelType.Fixed);
                        mv.GetNeuronAt("nMtch").添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), curRow).Id, -1, 突触.modelType.Fixed);
                        mv.GetNeuronAt(Array.IndexOf(cols, "thing"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "nMtch"), curRow).Id, 1, 突触.modelType.Fixed);
                        mv.GetNeuronAt("'1'").添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "nMtch"), curRow).Id, -1, 突触.modelType.Fixed);

                        mv.GetNeuronAt(Array.IndexOf(cols, "nMtch"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), curRow).Id, -1, 突触.modelType.Fixed);
                        mv.GetNeuronAt(Array.IndexOf(cols, "thing"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "head"), curRow).Id, 1, 突触.modelType.Fixed);
                        mv.GetNeuronAt(Array.IndexOf(cols, "head"), curRow).添加突触(nBeginning.Id, 1, 突触.modelType.Fixed);
                        mv.GetNeuronAt(Array.IndexOf(cols, "attrib"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "alt"), j).Id, 1, 突触.modelType.Fixed);

                        if (i != 0)
                            mv.GetNeuronAt(Array.IndexOf(cols, "head"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), curRow).Id, -1, 突触.modelType.Fixed);

                        if (i < sequenceItems.Length - 1)
                        {
                            //handle playing a sequence
                            mv.GetNeuronAt(Array.IndexOf(cols, "next"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "thing"), nextRow).Id, 1, 突触.modelType.Fixed);
                            mv.GetNeuronAt(Array.IndexOf(cols, "next"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), nextRow).Id, 1, 突触.modelType.Fixed);

                            //handle searching
                            mv.GetNeuronAt(Array.IndexOf(cols, "nMtch"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), nextRow).Id, 1, 突触.modelType.Fixed);
                        }
                        else
                        {
                            //stop the playback
                            mv.GetNeuronAt(Array.IndexOf(cols, "next"), curRow).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "next"), 0).Id, -1, 突触.modelType.Fixed);
        ...
        public override void Initialize()
        {
            ...
            n0.标签名 = "'1'";
            n0.添加突触(n0.Id, 1, 突触.modelType.Fixed);
            n0.SetValue(1);
            ...
            for (int i = 0; i < cols.Length; i++)
            {
                    ...
                    cols[i] == "say")
                {
                    ...
                    n.添加突触(n1.Id, -1, 突触.modelType.Fixed);
                    n0.添加突触(n1.Id, 1, 突触.modelType.Fixed);
                    for (int j = 2; j < mv.Height; j++)
                    {
                        n1.添加突触(mv.GetNeuronAt(i, j).Id, -1, 突触.modelType.Fixed);
                    }
                }
            }
            ...
            for (int j = 2; j < mv.Height; j++)
            {
                nClr.添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), j).Id, -1, 突触.modelType.Fixed);
            }
            mv.GetNeuronAt("recur").添加突触(nClr.Id, 1, 突触.modelType.Fixed);

            //put in all the horizontal synapses
            for (int j = 2; j < mv.Height; j++)
            {
                mv.GetNeuronAt(Array.IndexOf(cols, "in"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "in"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).Id, 10, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "this"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "attrib"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "anyAttr"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "allAttr"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "parent"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "child"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "next"), j).Id, 1, 突触.modelType.Fixed);


                mv.GetNeuronAt(Array.IndexOf(cols, "this"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "out"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "alt"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "out"), j).Id, 1, 突触.modelType.Fixed);
                //na.GetNeuronAt(Array.IndexOf(cols, "out"), j).AddSynapse(na.GetNeuronAt(Array.IndexOf(cols, "in"), j).Id, -1, false);
                mv.GetNeuronAt(Array.IndexOf(cols, "out"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "recur"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "recur"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), j).Id, 2, 突触.modelType.Fixed); //2 because the out is suppressing
                mv.GetNeuronAt(Array.IndexOf(cols, "match"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).Id, 1, 突触.modelType.Fixed);



                mv.GetNeuronAt(Array.IndexOf(cols, "out"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "say"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "out"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "0"), j).Id, 1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "0"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "say"), j).Id, -1, 突触.modelType.Fixed);

                mv.GetNeuronAt(Array.IndexOf(cols, "next"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "in"), j).Id, -1, 突触.modelType.Fixed);
                mv.GetNeuronAt(Array.IndexOf(cols, "next"), j).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "thing"), j).Id, -30, 突触.modelType.Fixed);
            }
            //make som coluimns into an always-fire
            mv.GetNeuronAt(Array.IndexOf(cols, "next"), 0).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "next"), 0).Id, 1, 突触.modelType.Fixed);
            mv.GetNeuronAt(Array.IndexOf(cols, "say"), 0).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "say"), 0).Id, 1, 突触.modelType.Fixed);
            mv.GetNeuronAt(Array.IndexOf(cols, "attrib"), 0).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "attrib"), 0).Id, 1, 突触.modelType.Fixed);
            mv.GetNeuronAt(Array.IndexOf(cols, "match"), 0).添加突触(mv.GetNeuronAt(Array.IndexOf(cols, "match"), 0).Id, 1, 突触.modelType.Fixed);
