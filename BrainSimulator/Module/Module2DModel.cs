    ...
    public class Module2DModel : ModuleBase
    {
        ...
        public override void Fire()
        {
            ...
            if (obstacle != null)
            {
                ...
                跨语言接口.FindDistanceToSegment(new Point(0, 0), s.P1.P, s.P2.P, out Point closest);
        ...
        private Thing MergeNewSegment(Segment s)
        {
            ...
            foreach (Thing t in UKSSegments)
            {
                ...
                float d1 = 跨语言接口.DistancePointToLine(s.P1.P, segment.P1.P, segment.P2.P);
                float d2 = 跨语言接口.DistancePointToLine(s.P2.P, segment.P1.P, segment.P2.P);
                ...
                Point closest;
                d1 = (float)跨语言接口.FindDistanceToSegment(s.P1.P, segment.P1.P, segment.P2.P, out closest);
                d2 = (float)跨语言接口.FindDistanceToSegment(s.P2.P, segment.P1.P, segment.P2.P, out closest);
        ...
        public bool AddSegmentFromTouch(PointPlus P1, PointPlus P2, PointPlus motion, int arm)
        {
            ...
            ColorInt theColor = 跨语言接口.ColorToInt(Colors.Wheat);
            Segment newSegment = new Segment() { P1 = P1, P2 = P2, theColor = theColor };
            //OrderSegment(newSegment);
        ...
        public Thing GetNearestThing(Angle theta, out float dist)
        {
            ...
            foreach (Thing t in UKSSegments)
            {
                ...
                跨语言接口.FindIntersection(new Point(-.2, 0), pv.P, s.P1.P, s.P2.P,
        ...
        public Segment FindGreen()
        {
            ...
            {
                Segment s = SegmentFromUKSThing(t);
                if (s.theColor == 跨语言接口.ColorToInt(Colors.Green))
                    return s;
            }
            return null;
        }
        ...
        public Segment GetNearestObject()
        {
            ...
            {
                s = SegmentFromUKSThing(UKSSegments[i]);
                跨语言接口.FindDistanceToSegment(new Point(0, 0), s.P1.P, s.P1.P, out Point closest);
        ...
        {
            模块视图 naUKS = theNeuronArray.FindModuleByLabel("Module2DUKS");
            ...
            模块视图 naVision = theNeuronArray.FindModuleByLabel("Module2DVision");
            ...
            for (int i = 0; i < possibleViewAngles; i++)
            {
                ...
                foreach (Thing t in UKSSegments)
                {
                    Segment s = SegmentFromUKSThing(t);
                    if (s == null) continue;
                    跨语言接口.FindIntersection(new Point(0, 0), P.P, s.P1.P, s.P2.P,
        ...
        public PointPlus CanISGoStraightTo(PointPlus midPoint, out Segment obstacle)
        {
            ...
            {
                Segment s = SegmentFromUKSThing(t);
                跨语言接口.FindIntersection(new Point(0, 0), midPoint.P, s.P1.P, s.P2.P,
                    out bool lines_intersect, out bool segments_intersect,
                    out Point intersection, out Point close_p1, out Point closep2, out double collisionAngle);
                if (segments_intersect)
                {
        ...
        private static int CompareSegmentsByDistance(Thing t1, Thing t2)
        {
            ...
            if (s1 == null || s2 == null)
                return 0;
            float d1 = (float)跨语言接口.FindDistanceToSegment(s1);
            float d2 = (float)跨语言接口.FindDistanceToSegment(s2);
            if (d1 > d2) return 1;
            if (d1 == d2)
            {
        ...
        public List<Thing> NearbySegments(int max = 1)
        {
            ...
            float d = (float)跨语言接口.FindDistanceToSegment(s);
            for (int i = 1; i < retVal.Count; i++)
            {
                s = SegmentFromUKSThing(retVal[i]);
                float d1 = (float)跨语言接口.FindDistanceToSegment(s);
                matches++;
                if (i >= max && Abs(d - d1) > .1)
                    break;
                d = d1;
        ...
        ******************************** */
        ...
        public override void Initialize()
        {
            ...
            mv.GetNeuronAt(0, 0).标签名 = "New";
            mv.GetNeuronAt(1, 0).标签名 = "Change";
            AddLabel("Obstacle");
            UpdateDialog();
        }
    }
