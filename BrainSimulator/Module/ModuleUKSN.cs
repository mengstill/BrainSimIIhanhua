    ...
    public class ModuleUKSN : ModuleUKS
   {
        ...
        public override void DeleteThing(Thing t)
        {
            ...
            if (i > -1)
            {
                ...
                {
                    神经元 targetNeuron = mv.GetNeuronAt(j - 1);
                    神经元 sourceNeuron = mv.GetNeuronAt(j);
                    MainWindow.thisWindow.此神经元数组视图.MoveOneNeuron(sourceNeuron, targetNeuron,false);
                }
                //repeat this process for the output array
                模块视图 naModule = theNeuronArray.FindModuleByLabel("KBOut");
                if (naModule != null)
                {
                    for (int j = i + 1; j < naModule.NeuronCount; j++)
                    {
                        神经元 targetNeuron = naModule.GetNeuronAt(j - 1);
                        神经元 sourceNeuron = naModule.GetNeuronAt(j);
                        MainWindow.thisWindow.此神经元数组视图.MoveOneNeuron(sourceNeuron, targetNeuron,false);
                    }
                }
            }
        ...
        public override void Initialize()
        {
            ...
            模块视图 na1 = theNeuronArray.FindModuleByLabel("KBOut");
            if (na1 != null)
                foreach (神经元 n in na1.Neurons)
                {
                    n.删除所有突触();
                }
            ...
            模块视图 naPhonemes = theNeuronArray.FindModuleByLabel("ModuleSpeakPhonemes");
            if (naPhonemes != null)
            {
                神经元 n2 = GetNeuron(Labeled("SayRnd"));
                神经元 n3 = naPhonemes.GetNeuronAt("BabyTalk");
                if (n2 != null && n3 != null)
                    n3.添加突触(n2.Id, 1);
                Thing parent = Labeled("Vowel");
                foreach (神经元 n in naPhonemes.Neurons)
                {

                    if (n.标签名 == "Conson.") parent = Labeled("Consonant");
                    if (n.标签名.Length == 1)
                    {
                        string label = "spk" + n.标签名;
                        Thing pn = AddThing(label, new Thing[] { parent });
                        神经元 n1 = GetNeuron(pn, false);
                        if (n1 != null)
                        {
                            n1.添加突触(n.Id, 1);
                        }
        ...
        public 神经元 GetNeuron(Thing t, bool input = true)
        {
            int i = UKS.IndexOf(t);
            if (i == -1) return null;
            if (input)
        ...
        public bool Fired(Thing t, long maxPastCycles = 1, bool firedInput = true, long minPastCycles = 0)
        {
            ...
            神经元 n = naModule.GetNeuronAt(i);
            if (n == null) return false;
            long timeSinceLastFire = MainWindow.此神经元数组.Generation - n.LastFired;
